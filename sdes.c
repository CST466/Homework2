...
SDES_Encrypt(char plain_text, SDES_KEYS keys)
{
	...
	char xor_ip_left_nibble_with_p4 = sdes_p4_out ^ (initial_permuation_out & 0xF0) >> 4);
	char combine_ip_right_nibble_with_above = (initial_permuation_out & 0x0F) | (xor_ip_left_nibble_with_p4 << 4);
	char swap_two_halves = ((combine_ip_right_nibble_with_above & 0xF0) >> 4) | ((combine_ip_right_nibble_with_above & 0x0F) << 4);
	char ep_of_right_nibble_swap = SDES_EP(swap_two_halves);
	...
	char xor_p4_on_sbox2_out_with_earlier_swap_left_nibble = p4_on_sbox2_out ^ ((swap_two_halves & 0xF0) >> 4);
	char replace_left_nibble_of_first_swap = (swap_two_halves & 0x0F) | (xor_p4_on_sbox2_out_with_earlier_swap_left_nibble << 4);
